...
public struct UnitSnapshotData : ISnapshotData<UnitSnapshotData>
{
    public uint tick;
    ...
    public int GetPlayerIdValue(GhostDeserializerState deserializerState)
    {
        return (int)PlayerIdValue;
    }

    public int GetPlayerIdValue()
    {
        return (int)PlayerIdValue;
    }

    public void SetPlayerIdValue(int val, GhostSerializerState serializerState)
    {
        PlayerIdValue = (int)val;
    }

    public void SetPlayerIdValue(int val)
    {
        PlayerIdValue = (int)val;
    }
    ...
    public void PredictDelta(uint tick, ref UnitSnapshotData baseline1, ref UnitSnapshotData baseline2)
    {
        var predictor = new GhostDeltaPredictor(tick, this.tick, baseline1.tick, baseline2.tick);
        PlayerIdValue = predictor.PredictInt(PlayerIdValue, baseline1.PlayerIdValue, baseline2.PlayerIdValue);
    ...
    public void Serialize(int networkId, ref UnitSnapshotData baseline, ref DataStreamWriter writer, NetworkCompressionModel compressionModel)
    {
        changeMask0 = (PlayerIdValue != baseline.PlayerIdValue) ? 1u : 0;
        ...
        bool isPredicted = GetPlayerIdValue() == networkId;
        ...
        if (isPredicted)
        {
            if ((changeMask0 & (1 << 0)) != 0)
                writer.WritePackedIntDelta(PlayerIdValue, baseline.PlayerIdValue, compressionModel);
        }
    }

    public void Deserialize(uint tick, ref UnitSnapshotData baseline, ref DataStreamReader reader,
        NetworkCompressionModel compressionModel)
    {
        ...
        {
            if ((changeMask0 & (1 << 0)) != 0)
                PlayerIdValue = reader.ReadPackedIntDelta(baseline.PlayerIdValue, compressionModel);
            else
                PlayerIdValue = baseline.PlayerIdValue;
        }
    }

    public void Interpolate(ref UnitSnapshotData target, float factor)
    {
