...
public static class MovementOrder
{
    ...
    public unsafe class HandleRequest : JobComponentSystem
    {
        ...
        private NativeArray<AStarPathfinding.Neighbour> neighbours;
        ...
        protected override void OnCreate()
        {
            ...
            neighbours = AStarPathfinding.Neighbour.Create(Allocator.Persistent);
        }

        protected override void OnDestroy()
        {
            AStarPathfinding.Neighbour.Destroy(ref neighbours);
        }
        ...
        protected override JobHandle OnUpdate(JobHandle inputDeps)
        {
            ...
            var neighboursRef = neighbours;

            // Execute job
            inputDeps = Entities
                .WithAll<NavigationAgent>()
                .WithNone<Waypoint>()
                .ForEach((Entity entity, int entityInQueryIndex, Request req, ref Translation translation) =>
                {
                    ...
                    if (path[cursor] != -1)
                    {
                        ...
                        while (path[cursor] != -1)
                        {
                            ...
                            var worldPos = transform.GetWorldPos(NavigationMap.ToCenterPos(coord, navMap.NodeSize));
    ...
    public unsafe struct AStarPathfinding
    {
        ...
        public static bool Solve(float3 from, float3 to, ref NavigationMap map, ref NativeArray<Neighbour> neighbourOffsets, out NativeArray<int> cameFrom, out int firstIndex)

        {
            // Currently only uses X & Z coordinates (no Y-axis movement)
            int endIdx = NavigationMap.GetIndex(PositionUtil.SetY(to, from.y), map.NodeSize, map.Size);
            int startIdx = NavigationMap.GetIndex(from, map.NodeSize, map.Size);
            ...
            if (!NavigationMap.NotOutOfBounds(map.NodesPtr[endIdx].Coord, map.Size) || !map.NodesPtr[endIdx].Walkable) return false;
            ...
            while (openList.Length > 0)
            {
                ...
                for (int i = 0; i < neighbourOffsets.Length; ++i)
                {
                    ...
                    if (!NavigationMap.NotOutOfBounds(neighbourCoord, map.Size)) continue;

                    int neighbourIdx = NavigationMap.GetIndex(neighbourCoord, map.Size);
