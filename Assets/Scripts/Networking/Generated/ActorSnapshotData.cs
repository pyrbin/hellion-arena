...
public struct ActorSnapshotData : ISnapshotData<ActorSnapshotData>
{
    public uint tick;
    ...
    public int GetActorPlayerId(GhostDeserializerState deserializerState)
    {
        return (int)ActorPlayerId;
    }
    public int GetActorPlayerId()
    {
        return (int)ActorPlayerId;
    }
    public void SetActorPlayerId(int val, GhostSerializerState serializerState)
    {
        ActorPlayerId = (int)val;
    }
    public void SetActorPlayerId(int val)
    {
        ActorPlayerId = (int)val;
    }
    ...
    public void PredictDelta(uint tick, ref ActorSnapshotData baseline1, ref ActorSnapshotData baseline2)
    {
        var predictor = new GhostDeltaPredictor(tick, this.tick, baseline1.tick, baseline2.tick);
        ActorPlayerId = predictor.PredictInt(ActorPlayerId, baseline1.ActorPlayerId, baseline2.ActorPlayerId);
    ...
    public void Serialize(int networkId, ref ActorSnapshotData baseline, ref DataStreamWriter writer, NetworkCompressionModel compressionModel)
    {
        changeMask0 = (ActorPlayerId != baseline.ActorPlayerId) ? 1u : 0;
        ...
        if ((changeMask0 & (1 << 0)) != 0)
            writer.WritePackedIntDelta(ActorPlayerId, baseline.ActorPlayerId, compressionModel);
    ...
    public void Deserialize(uint tick, ref ActorSnapshotData baseline, ref DataStreamReader reader,
        NetworkCompressionModel compressionModel)
    {
        this.tick = tick;
        changeMask0 = reader.ReadPackedUIntDelta(baseline.changeMask0, compressionModel);
        if ((changeMask0 & (1 << 0)) != 0)
            ActorPlayerId = reader.ReadPackedIntDelta(baseline.ActorPlayerId, compressionModel);
        else
            ActorPlayerId = baseline.ActorPlayerId;
        if ((changeMask0 & (1 << 1)) != 0)
        {
    ...
    public void Interpolate(ref ActorSnapshotData target, float factor)
    {
